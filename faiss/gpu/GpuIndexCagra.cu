...
void GpuIndexCagra::copyTo(faiss::IndexHNSWCagra* index) const {
    ...
    // Add vectors to storage directly without triggering HNSW build
    index->storage->add(n_train, train_dataset);
    index->ntotal = n_train;

    // Set up HNSW structure directly without calling add()
    index->init_level0 = false;
    index->base_level_only = true;  // Only use level 0, no upper levels

    // Prepare the HNSW structure for the existing data
    ...
    // Copy the graph structure directly
